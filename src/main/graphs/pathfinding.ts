import { CACHE, ROOM_MATRIX_TIMEOUT, ROOM_PATH_TIMEOUT } from 'main/consts'
...
export function getPath(
  ...
  // Find path in cache
  const key = `${CACHE.PATH}${packRoomPosition(start)}${packRoomPosition(end)}`
  const cache = Memory.cache[key]
  if (cache) {
    // cache.t = TIMEOUTS.ROOM_PATH
    return [...(cache.r as string)].map(x => +x as DirectionConstant)
  }

  // If no custom costCallback set, use cached matrix or generate one
  if (!options.costCallback) {
    const matrixKey = `${CACHE.HAVE_MATRIX}${packRoomName(start.roomName)}`
    let matrix = Memory.cache[matrixKey]?.r as number[] | undefined
    if (!matrix) {
  ...
  for (let x = 0; x < 50; x++)
    for (let y = 0; y < 50; y++) {
      ...
      // If not moving creep set as unpreferrable
        ...
        look.some(x => x.creep && !x.creep.spawning && (!x.creep.my || !x.creep.memory.move))
      )
        matrix.set(x, y, 50)
